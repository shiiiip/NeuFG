...
class NeuFGRenderer:
    ...
    def stepfunc(self, n, t):
        if (n == 0):
            h = (t == 0) * 0.5 + (t > 0) * 1
        else:
            h1 = self.stepfunc(n - 1, (t + 1))
            h2 = self.stepfunc(n - 1, (t - 1))
            h = 0.5 * ((1 + t / n) * h1 + (1 - t / n) * h2)
        return h

    def stepfuncGen(self, n, delta, t):
        return self.stepfunc(n, n * t / delta)

    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, delta):
        batch_size, n_samples = z_vals.shape
        ...
        prev_esti_sdf = (mid_sdf - cos_val * dist * 0.5) * -1.0
        next_esti_sdf = (mid_sdf + cos_val * dist * 0.5) * -1.0

        prev_cdf = self.stepfuncGen(3, 1 / delta, prev_esti_sdf)
        next_cdf = self.stepfuncGen(3, 1 / delta, next_esti_sdf)

        alpha = (next_cdf - prev_cdf + 1e-5) / (1 - prev_cdf + 1e-5)
    ...
    def render_core(self,
        ...
        # Single parameter
        delta = deviation_network(torch.zeros([1, 3]))[:, :1].clip(1e-6, 1e6)
        delta = delta.expand(batch_size * n_samples, 1)

        true_cos = (dirs * gradients).sum(-1, keepdim=True)
        iter_cos = -(F.relu(-true_cos * 0.5 + 0.5) * (1.0 - cos_anneal_ratio) +
                     F.relu(-true_cos) * cos_anneal_ratio)

        estimated_next_sdf = (sdf + iter_cos * dists.reshape(-1, 1) * 0.5) * -1.0
        estimated_prev_sdf = (sdf - iter_cos * dists.reshape(-1, 1) * 0.5) * -1.0

        prev_cdf = self.stepfuncGen(3, 1 / delta, estimated_prev_sdf)
        next_cdf = self.stepfuncGen(3, 1 / delta, estimated_next_sdf)
    ...
    def render(self, rays_o, rays_d, near, far, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0, intrinsics=None, intrinsics_inv=None, poses=None, images=None):
        ...
        delta_val = ret_fine['delta_val'].reshape(batch_size, n_samples).mean(dim=-1, keepdim=True)
